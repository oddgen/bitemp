...
import org.oddgen.bitemp.sqldev.templates.CreateHistoryViewInsteadOfTrigger
import org.oddgen.bitemp.sqldev.templates.CreateObjectType
import org.oddgen.bitemp.sqldev.tests.AbstractJdbcTest

class CreateHistoryViewInsteadOfTriggerTest extends AbstractJdbcTest {
	...
	def existingBiTemporal() {
		...
		val template = new CreateHistoryViewInsteadOfTrigger
		val script = template.compile(model).toString
		for (stmt : script.statements) {
			jdbcTemplate.execute(stmt)
		}
		Assert.assertEquals("VALID", getObjectStatus("TRIGGER", "T1_HV_TRG"))
	}
	...
	def static void tearDown() {
		...
		try {
			jdbcTemplate.execute("DROP VIEW t1_hv")
		} catch (Exception e) {
		}
		try {
		...
		try {
			jdbcTemplate.execute("DROP VIEW dept_hv")
		} catch (Exception e) {
		}
	}
}

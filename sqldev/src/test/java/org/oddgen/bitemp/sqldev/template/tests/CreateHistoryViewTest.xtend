...
class CreateHistoryViewTest extends AbstractJdbcTest {

	@Test
	def newBiTemporal() {
		val template = new CreateHistoryView
		...
		Assert.assertEquals("INVALID", getObjectStatus("VIEW", "DEPT_HV"))
		val colCount = jdbcTemplate.queryForObject('''
			SELECT count(*)
			  FROM user_tab_columns
			 WHERE table_name = ?
		''', Integer, #["DEPT_HV"])
		Assert.assertEquals(7, colCount)
	}

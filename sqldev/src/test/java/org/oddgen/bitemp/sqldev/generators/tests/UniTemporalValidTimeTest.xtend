...
class UniTemporalValidTimeTest extends AbstractJdbcTest {
	...
	def genDeptBased() {
		...
		jdbcTemplate.execute('''
			INSERT 
			  INTO d2_hv 
			VALUES (NULL, SYSDATE, SYSDATE+10, 50, 'TEST', 'Zürich')
		''')
		Assert.assertEquals(5, getCount("D2", ""))
		...
		jdbcTemplate.execute('''
			INSERT 
			  INTO d2_hv 
			VALUES (NULL, SYSDATE, SYSDATE+400, 60, 'TEST2', 'BERN')
		''')
		...
		jdbcTemplate.execute('''
			INSERT 
			  INTO d2_hv 
			VALUES (NULL, SYSDATE+200, NULL, 60, 'TEST2', 'BERN')
		''')
		...
		jdbcTemplate.execute('''
			INSERT 
			  INTO d2_hv 
			VALUES (NULL, NULL, SYSDATE, 60, 'TEST2', 'BERN')
		''')
		Assert.assertEquals(1, getCount("D2", "WHERE deptno = 60 and is_deleted$ IS NULL"))
	...
	def alwaysGeneratedIdentityColumn() {
		...
		jdbcTemplate.execute('''
			INSERT 
			  INTO d3_hv 
			VALUES (NULL, NULL, NULL, 200 /* ignored */, 'Text')
		''')
	...
	def genBulkLoad() {
		...
		jdbcTemplate.execute('''
			...
			BEGIN
			   DELETE FROM d2_sta$;
			   ins(null, null, null, 60, 'HELLO');
			   ins(DATE '2020-01-01', null, 1, 20, null);
			   ins(DATE '2016-01-03', DATE '2017-01-01', null, 40, NULL);
			END; 
		''')
		Assert.assertEquals(3, getCount("D2_STA$", ""))
		jdbcTemplate.execute('''
			BEGIN
				d2_api.delta_load (in_reject_limit => 'unlimited');
			END;
		''')
		Assert.assertEquals(1, getCount("D2_LOG$", ""))
		Assert.assertEquals(5, getCount("D2", ""))
		Assert.assertEquals(15, getCount("D2_HT", ""))
		Assert.assertEquals(1, getCount("D2_HT", "WHERE deptno = 20 and is_deleted$ = 1"))
